...
package com.service.service.controller.transaction.activity;
...
import com.service.service.service.transaction.activity.ActivityHistoryService;
import com.service.service.dto.transaction.activity.ActivityHistoryDTO;
import com.service.service.entity.transaction.activity.ActivityHistoryEntity;


@RestController
@RequestMapping("/activity-history")
public class ActivityHistoryController extends CommonUtil { 
 
     @Autowired
    ActivityHistoryService service;

    String path = "activity-history";

    @PostMapping
    ResponseEntity<ResponseUtils> create(@RequestBody @Valid ActivityHistoryDTO dto) {
        ActivityHistoryEntity entity = service.save(ConvertModel(dto, ActivityHistoryEntity.class));
        ActivityHistoryDTO res = ConvertModel(entity, ActivityHistoryDTO.class);
    ...
    @PutMapping()
    ResponseEntity<ResponseUtils> update(@RequestBody @Valid ActivityHistoryDTO dto) {
        ActivityHistoryEntity entity = service.update(dto.getId(),
                ConvertModel(dto, ActivityHistoryEntity.class));
        Object res = (entity == null ? (dto.getId() + " Not Found")
                : ConvertModel(entity, ActivityHistoryDTO.class));
        return ResponseEntity
                .status(HttpStatus.OK)
                .contentType(MediaType.APPLICATION_JSON)
                .body(
        ...
        ResponseEntity<ResponseUtils> getAll(
                ...
                Object res = null;
                Page<ActivityHistoryEntity> dataPage = null;
                if (page != null) {
                        dataPage = service
